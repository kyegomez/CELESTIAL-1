...
from cstx.decoders.language import LlamaForCausalLM
from cstx.decoders.video import TextToVideoSDPipeline
from cstx.ImageBind import *
from cstx.ImageBind import data
from cstx.utils.qformer import BertConfig, BertLMHeadModel
from cstx.utils.utils import *


class TextFcLayer(nn.Module):
    #layers used in mapping text embeddings
    ...
    def __init__(
        ...
        if mode == "linear":
            ...
            self.query_embs = nn.Parameter(
                torch.rand(1, num_input_tokens, hidden_dim)
            )
        elif mode == "qformer":
            ...
            self.Qformer, self.query_embs = self.init_Qformer(
    ...
    def forward(
        ...
        if isinstance(self.model, nn.ModuleList):
            ...
            x = self.tfm(
                x,
                self.query_embs.repeat(x.shape[0], 1, 1)
            )
...
class Celestial(nn.Module):
    ...
    def prompt_wrap(
        ...
        if img_embeds is not None:
            ...
            ).to(self.device)  # bsz x (1+s1+1+s2) x embed_dim
            ...
            ).to(self.device)  # bsz x (1+s1+s2) x embed_dim
    ...
    def _train_with_mode(
        ...
        if stage == 2:
            ...
            input_ids, target_ids, attention_mask = process_batch_stage_3(
                self.llama_tokenizer, texts, self.max_length,
        ...
        if modality == 'text':
            ...
            [conversation for conversation in texts]
            ...
            if modality == 'image':
                ...
                text_prompt_embeddins = torch.stack(
                    text_prompt_embeddins, 
                    dim=0
                ).to(self.device)

                assert len(
                    text_prompt_embeddins.shape
                ) == 2, text_prompt_embeddins.shape

                text_prompt_embeddins = text_prompt_embeddins.view(
                    text_prompt_embeddins.size(0), 
                    1,
    ...
    def _prepare_image_embed(self, text, batch_size):
        ...
        if self.args['freeze_lm']:
            ...
            ).expand(batch_size, -1, -1)  # bsz x s2 x embed_dim
        ...
        with torch.no_grad():  # no tracking history
            ...
            for i in range(inputs['max_tgt_len']):
                ...
                for stop in stop_words_ids:
                    if out is not None and torch.all((stop == out[0][-len(stop):])).item():
                        stop_count += 1
    ...
    def generate_videos(
        ...
        for gen_idx in all_gen_idx:
            ...
            caption = \
                self.llama_tokenizer.batch_decode(
                    generated_ids[:, last_ret_idx:gen_idx], 
